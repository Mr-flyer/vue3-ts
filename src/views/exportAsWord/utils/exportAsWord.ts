...
const handleImage = (ele: HTMLElement, cloneEle: HTMLElement) => {
  ...
  cloneEle.querySelectorAll(".ProseMirror img").forEach((item) => {
    if (!["ProseMirror-separator", "image-err", "resizer-img"].includes(item.className)) return;
...
const removeWhiteBox = (cloneEle: HTMLElement) => {
  const separators: NodeListOf<Element> = cloneEle.querySelectorAll(".ProseMirror-separator");
...
const handleMind = async (ele: HTMLElement, cloneEle: HTMLElement) => {
  const mindDOM = ele.querySelectorAll(`.mind-snapshot`) as unknown as HTMLElement[];
  const promise = [...mindDOM].map((mind) => Html2canvas(mind.firstChild as HTMLElement));
...
const handleUlStyle = (cloneEle: HTMLElement, uiLevel: number) => {
  const changeTask2P = (div: HTMLElement | Element, parent?: HTMLElement | Element) => {
    const p = document.createElement("p");
    p.innerHTML = div.innerHTML;
    parent ? parent.insertAdjacentElement("afterend", p) : div.insertAdjacentElement("afterend", p);
    return p;
  };
  const changeDiv2Ul = (
    ...
    liOrp.querySelectorAll(".list-content").forEach((content) => {
      const span = document.createElement("span");
      span.innerHTML = (content.firstChild as HTMLElement)?.innerHTML || "<li></li>";
      liOrp && (liOrp.style.textAlign = (content.firstChild as HTMLElement)?.style?.textAlign);
